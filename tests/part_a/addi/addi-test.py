...
if len(sys.argv) >= 2:
  if sys.argv[1] == "single":
    tests.append(("cpu addi (single-stage) test", "cpu-addi", "single"))
  elif sys.argv[1] == "pipelined":
    tests.append(("cpu addi (two-stage) test", "cpu-addi", "pipelined"))
  else:
    raise ValueError("Invalid addi test type: %s" % sys.argv[1])
else:
  raise ValueError("You must specify an addi test type: single, pipelined")
